...
for f in $outputFolder/$foldername/size_bins/*.bed.gz;
	do
		...
		#bsub -q short -W 02:00 -e $HOME/lsf_jobs/LSB_%J.err -o $HOME/lsf_jobs/LSB_%J.log -n 1 -R select[ib] -R rusage[mem=8192] -N -u $USER -w bin2aggregate$filename_bin -J aggr2norm$filename_bin "perl ~/scripts/git/ATACpipe/perl/aggregate2norm.pl $outputFolder/$foldername/aggregateVectors/$filename_bin.$aggregateName.aggregateVector.txt";
		#bsub -q short -W 02:00 -e $HOME/lsf_jobs/LSB_%J.err -o $HOME/lsf_jobs/LSB_%J.log -n 1 -R select[ib] -R rusage[mem=8192] -N -u $USER -J mv2norm$filename_bin -w aggr2norm$filename_bin "mv $outputFolder/$foldername/aggregateVectors/$filename_bin.$aggregateName.aggregateVector.txt.norm.vector.txt $outputFolder/$foldername/norm_aggregateVectors/.";
