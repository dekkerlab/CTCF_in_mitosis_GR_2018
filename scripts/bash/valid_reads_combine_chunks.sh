...
bsub -q short -W 04:00 -e $HOME/lsf_jobs/LSB_%J.err -o $HOME/lsf_jobs/LSB_%J.log -n 1 -R select[ib] -R rusage[mem=1000] -N -u $USER -J cat2unique$name -w cat2sort$name "perl $HOME/scripts/git/ATACpipe/perl/pebam2unique.pl $inputFolder/ALL/$name.ALL.sort.bam"; 
...
bsub -q short -W 04:00 -e $HOME/lsf_jobs/LSB_%J.err -o $HOME/lsf_jobs/LSB_%J.log -n 1 -R select[ib] -R rusage[mem=1000] -N -u $USER -J unique2lengths$name -w unique2bam$name "perl $HOME/scripts/git/ATACpipe/perl/pebam2lengths.pl $inputFolder/ALL/$name.ALL.sort.bam.unique.bam";

bsub -q short -W 04:00 -e $HOME/lsf_jobs/LSB_%J.err -o $HOME/lsf_jobs/LSB_%J.log -n 1 -R select[ib] -R rusage[mem=2000] -N -u $USER -J lengths2plot$name -w unique2lengths$name "Rscript $HOME/scripts/git/ATACpipe/R/lengths.R $inputFolder/ALL/$name.ALL.sort.bam.unique.bam.LENGTHS $name";
